...
from data_loader import train_data_haar
...
parser.add_argument("--valid_freq",default=10,type=int,help="Decide how many epochs a validation")
parser.add_argument("--Alpha",default=0.2,type=float,help="LF loss parament")
parser.add_argument("--in_channels", default=3, type=int, help="3 channels * 2 = 6")
parser.add_argument("--num_blocks",default=8,type=int,help="Num of InvBlock")
...
def main():
    ...
    train_set = train_data_haar('./Dataset/train/data_crop',
                                './Dataset/train/label_crop', )

    valid_set = train_data_haar('./Dataset/valid/data_crop',
    ...
    save_checkpoint(model1,epoch_min*opt.valid_freq,loss_valid_list[argmin(loss_valid_list)])
    save_checkpoint(model1,opt.nEpochs,loss_valid_list[-1])
...
def train(training_data_loader, optimizer, model1, criterion, epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        Ref, CPL, Ref_wt, CPL_wt = Variable(batch[0]), Variable(batch[1], requires_grad=False),Variable(batch[2]),Variable(batch[3],requires_grad=False)
        ...
        LF,HF_ = model1(Ref)
        CPL_LR = CPL_wt[:, 0:1, :, :]
        loss_fwd1 = criterion(CPL_LR,LF)
        CPL_HR = CPL_wt[:, 1:4, :, :]
        loss_fwd2 = criterion(CPL_HR, HF_)
        loss_fwd = 0.5*loss_fwd1 + 0.5*loss_fwd2

        pred = model1.reverse(LF,HF_)
        loss_bkd = criterion(CPL,pred)

        loss = 0.9*loss_bkd + 0.1*loss_fwd
        ...
        if iteration%100 == 0:
            print("===> Epoch[{}]({}/{}): Loss: {:.10f}".format(epoch, iteration, len(training_data_loader), loss))

    return loss

def save_checkpoint(model1, epoch,loss):
    model_out_path = "./Weights/" + "InvJND_{}__{}.pth".format(epoch,loss)
...
def valid(epoch,valid_data_loader,criterion):
    for iteration, batch in enumerate(valid_data_loader, 1):
        ...
        LFv,HF_v = model1(input_v)
        predv = model1.reverse(LFv,HF_v)

        loss_valid = criterion(predv,input_v)
